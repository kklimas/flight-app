...
export class FlightSearchComponent implements OnInit {
  ...
  handleFromLocation() {
    ...
    if (this.from.length > 3) {
      fetch(`http://localhost:8099/city-and-airport-search/${this.from}`)
  ...
  handleToLocation() {
    ...
    if (this.to.length > 3) {
      fetch(`http://localhost:8099/city-and-airport-search/${this.to}`)
      .then(response => response.json())
  ...
  onFindFlight() {
    ...
    if (this.date == "") {
      ...
      fetch(`http://localhost:8099/flight-search?originCode=${this.origin.iataCode}&destinationCode=${this.destination.iataCode}&dateOfDeparture=${this.date}`, {
      method: 'GET', 
      headers: {
  ...
  onBookFlight(flight: any) {
    ...
    fetch('http://localhost:8099/flight-confirmation', {
      method: 'POST', 
      headers: {
        'Content-Type': 'application/json',
      },
    ...
    .then(dataObject => {
      ...
      fetch('http://localhost:8099/flight-booking', {
        method: 'POST', 
        headers: {
          'Content-Type': 'application/json',
        },
